...
export default function CareersPageClient({ company, sections, jobs }: CareersPageClientProps) {
  ...
  const uniqueDepartments = Array.from(new Set(jobs.map(j => j.department).filter(Boolean))).sort();
      ...
      <div id="open-positions" className="bg-white py-12 sm:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-slate-50 rounded-xl p-4 sm:p-6 mb-8">
            ...
            {(searchTerm || locationFilter || departmentFilter || typeFilter) && (
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {typeFilter && (
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1 bg-white rounded-full text-sm text-slate-700 hover:bg-slate-100 transition"
                    >
                      {typeFilter.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                      <X className="w-3 h-3" />
                    </button>
                  )}
                </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-3 mb-4">
                        ...
                        <span className="flex items-center gap-1.5 text-sm text-slate-600">
                          ...
                          {job.job_type.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                        </span>
                        {job.department && (
                        ...
                        {job.salary_range && (
                          <span className="text-sm font-medium" style={{ color: company.primary_color }}>
                            {job.salary_range}
                          </span>
                        )}
                      </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        alert('Application form would open here. Integration with your ATS system required.');
                      }}
                    >
                      Apply Now
                    </button>
